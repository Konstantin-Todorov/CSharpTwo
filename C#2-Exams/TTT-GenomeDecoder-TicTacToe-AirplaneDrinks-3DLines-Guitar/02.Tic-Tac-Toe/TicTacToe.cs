    ...
    static void Main()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                if (line[j] == '-')
                {
                    board[i, j] = TicTacToePrediction.EmptyCell;
                }
                else if (line[j] == 'X')
                {
                    board[i, j] = TicTacToePrediction.PlayerX;
                }
                else if (line[j] == 'O')
                {
                    board[i, j] = TicTacToePrediction.PlayerO;
                }
            }
        }
        TicTacToePrediction prediction = new TicTacToePrediction(board);
        prediction.Predict();
        Console.WriteLine(prediction.playerXWins);
        Console.WriteLine(prediction.evenGames);
        Console.WriteLine(prediction.playerOWins);
    }
}
class TicTacToePrediction
{
    ...
    public TicTacToePrediction(byte[,] board)
    {
        this.board = board;
    }
