...
class GenomeDecoder
{
    static void Main()
    {
        ...
        string genomEncoded = Console.ReadLine();
        
        StringBuilder genome = new StringBuilder();
        int number = 0;
        for (int i = 0; i < genomEncoded.Length; i++)
        {
            if (char.IsDigit(genomEncoded[i]))
            {
                number =number * 10 + (genomEncoded[i] - '0');
            }
            else
            {
                if (number == 0)
                ...
                genome.Append(new string(genomEncoded[i], number));
                number = 0;
            }
        }
